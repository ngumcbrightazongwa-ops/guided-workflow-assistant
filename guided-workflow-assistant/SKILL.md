...
description: Build an in-app guided workflow assistant — a real task engine that walks users through completing actual work on the real screens (not a product tour or tooltip walkthrough). It navigates between pages, spotlights groups of real form fields, ticks them off live as they are filled, refuses to move on until the server proves the step is done, skips work already done, branches on questions, persists progress so a task survives refreshes and logouts, and records analytics on where people get stuck. Use this skill whenever the user wants a "workflow assistant", "guided workflow", "task wizard across pages", "onboarding that actually does the work", "walk the admin through adding X", "step-by-step helper", "command palette of tasks", "copilot for my dashboard", "resume where you left off", or asks to make an admin panel or customer portal easier to use — even if they only say "product tour" or "walkthrough" but describe users completing real tasks. Also use it when adding such an assistant to a second audience (e.g. a customer portal beside an admin), to a server-rendered (Blade, Livewire, Rails, Django) page, or when debugging one that highlights the wrong thing, covers the fields it points at, or will not notice what the user did.
---
